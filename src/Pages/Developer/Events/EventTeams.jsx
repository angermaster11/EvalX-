...
import { FiUsers, FiChevronDown, FiChevronUp, FiFolder } from "react-icons/fi";
import api from "@/utils/api";

export default function EventTeams({ event, responses, onJoin, onTeamChange }) {
  const [openTeam, setOpenTeam] = useState(null);
  const [currentUserId, setCurrentUserId] = useState(null);
  const [addUserMap, setAddUserMap] = useState({});
  const [inviteEmailMap, setInviteEmailMap] = useState({});
  const [pendingRequestsMap, setPendingRequestsMap] = useState({}); // local cache
  ...
  useEffect(() => {
    api.get("/dev/get-user").then((res) => setCurrentUserId(res.data.data.id)).catch(() => setCurrentUserId(null));
  }, []);

  useEffect(() => {
    if (!event) return;
    api.get(`/dev/events/${event.id || event._id}/teams/open`).then((res) => setOpenTeams(res.data.data)).catch(() => setOpenTeams(null));
  }, [event, onTeamChange, responses]);

  useEffect(() => {
    teams.forEach((t) => {
      if (t.requests && t.requests.length) {
        pendingRequestsMap[t._id || t.teamId || t.id] = t.requests.filter((r) => r.status === "pending");
      }
    });
  ...
  (responses?.rounds || []).forEach((r) => {
    (r.submissions || []).forEach((s) => {
      submissionCount[s.teamId] = (submissionCount[s.teamId] || 0) + 1;
    });
  });

  if (!responses)
    return <div className="text-sm text-neutral-400 animate-pulse">Loading teams…</div>;
  ...
  return (
    <div className="space-y-4">
          ...
          <div key={id} className="bg-white rounded-xl border border-neutral-200 p-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <div className="text-xs text-neutral-600 mt-1">{membersCount} members • { (team.leaderId && team.leaderId === currentUserId) ? "You lead" : "" }</div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => onJoin && onJoin(id)}
                  className="px-3 py-1 text-xs rounded-lg border border-neutral-300 bg-white hover:bg-neutral-100"
                >
                  Request to join
                </button>
              </div>
      ...
      {teams.map((team) => {
        ...
        const leader = team.leaderId || (team.members?.[0] || {}).userId;
        const isLeader = currentUserId && leader && currentUserId === leader;
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                {!isMember && !hasRequested && (
                  <button onClick={() => onJoin && onJoin(id)} className="px-3 py-1 text-xs rounded-lg border border-neutral-300 bg-white hover:bg-neutral-100">Request</button>
                )}
                ...
                {isLeader && (
                  <button onClick={async () => {
                    ...
                    try {
                      await api.delete(`/dev/events/${event.id || event._id}/teams/${id}`);
                      onTeamChange && onTeamChange();
                    } catch (err) {
                      alert(err?.response?.data?.detail || "Failed to delete team");
                    }
                  }} className="px-3 py-1 text-xs rounded-lg border border-red-300 bg-white hover:bg-red-50 text-red-600">Delete</button>
                )}

                <button className="p-2 rounded-lg hover:bg-neutral-100 transition" onClick={() => setOpenTeam((prev) => (prev === id ? null : id))}>
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25 }} className="mt-4 border-t border-neutral-200 pt-3">
                  ...
                  <div className="space-y-2">
                    {team.members?.map((m, idx) => {
                      const label = typeof m === "string" ? m : `${m.firstName || m.firstname || ""} ${m.lastName || m.lastname || ""} (${m.email || ""})`;
                        ...
                        <div key={idx} className="px-3 py-2 bg-neutral-50 rounded-lg border border-neutral-200 text-sm text-neutral-700 flex justify-between items-center gap-3">
                          ...
                          <div className="flex items-center gap-2">
                            {(isLeader || (currentUserId && currentUserId === memberId)) && (
                              <button onClick={async () => {
                                try {
                                  await api.post(`/dev/events/${event.id || event._id}/teams/${id}/members/remove`, new URLSearchParams({ userId: memberId }));
                                  onTeamChange && onTeamChange();
                                } catch (err) {
                                  alert(err?.response?.data?.detail || "Failed to remove member");
                                }
                              }} className="text-xs px-2 py-1 rounded bg-red-50 text-red-600 border border-red-100">
                                {currentUserId === memberId ? "Leave" : "Remove"}
                              </button>
                            )}
                          </div>
                        </div>
                      ...
                      <>
                        <div className="mt-3 grid grid-cols-1 gap-2">
                          <div className="flex gap-2">
                            ...
                            <button onClick={async () => {
                              const uid = addUserMap[id];
                              if (!uid) return alert("Enter userId to add");
                              try {
                                await api.post(`/dev/events/${event.id || event._id}/teams/${id}/members/add`, new URLSearchParams({ userId: uid }));
                                setAddUserMap((s) => ({ ...s, [id]: "" }));
                                onTeamChange && onTeamChange();
                              } catch (err) {
                                alert(err?.response?.data?.detail || "Failed to add member");
                              }
                            }} className="px-3 py-2 rounded-lg bg-black text-white text-sm">Add</button>
                          </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={async () => {
                              const email = inviteEmailMap[id];
                              if (!email) return alert("Enter email to invite");
                              try {
                                await api.post(`/dev/events/${event.id || event._id}/teams/${id}/invite`, new URLSearchParams({ email }));
                                setInviteEmailMap((s) => ({ ...s, [id]: "" }));
                                onTeamChange && onTeamChange();
                              } catch (err) {
                                alert(err?.response?.data?.detail || "Failed to invite member");
                              }
                            }} className="px-3 py-2 rounded-lg bg-white border border-neutral-300 text-sm">Invite</button>
                          </div>
                        </div>
                          ...
                          {(team.requests || []).filter(r => r.status === "pending").map((r) => (
                            <div key={r.requestId} className="flex items-center justify-between gap-3 px-3 py-2 bg-neutral-50 rounded-lg border border-neutral-200 mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                <button onClick={async () => {
                                  try {
                                    await api.post(`/dev/events/${event.id || event._id}/teams/${id}/requests/${r.requestId}/accept`);
                                    onTeamChange && onTeamChange();
                                  } catch (err) {
                                    alert(err?.response?.data?.detail || "Failed to accept request");
                                  }
                                }} className="px-3 py-1 text-xs rounded-lg bg-green-50 text-green-700 border border-green-100">Accept</button>

                                <button onClick={async () => {
                                  try {
                                    await api.post(`/dev/events/${event.id || event._id}/teams/${id}/requests/${r.requestId}/reject`);
                                    onTeamChange && onTeamChange();
                                  } catch (err) {
                                    alert(err?.response?.data?.detail || "Failed to reject request");
                                  }
                                }} className="px-3 py-1 text-xs rounded-lg bg-red-50 text-red-700 border border-red-100">Reject</button>
                              </div>
                            </div>
                          ))}
